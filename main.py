...
def eval_one_epoch(opt, model, test_dataset):
    ...
    if torch.cuda.current_device() == 0:  # Save only on main GPU
...
def train(opt):
    ...
    for n_epoch in range(opt['epoch']):
        ...
        if torch.cuda.current_device() == 0:  # Log only on main GPU
            writer.add_scalars('data/cost', {'train': epoch_cost / (n_iter + 1)}, n_epoch)
            print(f"training loss(epoch {n_epoch}): {epoch_cost / (n_iter + 1):.03f}, "
                  f"cls - {epoch_cost_cls / (n_iter + 1):.03f}, "
                  f"reg - {epoch_cost_reg / (n_iter + 1):.03f}, "
                  f"lr - {optimizer.param_groups[-1]['lr']:.06f}")
        ...
        if torch.cuda.current_device() == 0:  # Log only on main GPU
            ...
            print(f"testing loss(epoch {n_epoch}): {tot_loss:.03f}, "
                  f"cls - {cls_loss:.03f}, reg - {reg_loss:.03f}, mAP Avg - {IoUmAP_5:.03f}")
            
            state = {'epoch': n_epoch + 1, 'state_dict': model.module.state_dict()}  # Use module to access original model
            torch.save(state, f"{opt['checkpoint_path']}/{opt['exp']}_checkpoint_{n_epoch + 1}.pth.tar")
            if IoUmAP_5 > getattr(model.module, 'best_map', 0):
                model.module.best_map = IoUmAP_5
                torch.save(state, f"{opt['checkpoint_path']}/{opt['exp']}_ckp_best.pth.tar")
...
def eval_map_supnet(opt, dataset, output_cls, output_reg, labels_cls, labels_reg):
    ...
    for video_name in dataset.video_list:
        ...
        video_time = float(dataset.video_dict[video_name]["duration"])
        frame_to_time = 100.0 * video_time / duration
        conf_queue = torch.zeros((unit_size, num_class - 1)).cuda()
...
def test_frame(opt):
    ...
    if torch.cuda.current_device() == 0:  # Save only on main GPU
        print(f"testing loss: {tot_loss:.03f}, cls_loss: {cls_loss:.03f}, reg_loss: {reg_loss:.03f}")
    ...
    print(f"working time: {working_time:.2f}s, {total_frames / working_time:.2f}fps, {total_frames} frames")
...
def test_online(opt):
    ...
    for video_name in dataset.video_list:
        input_queue = torch.zeros((unit_size, opt['feat_dim'])).cuda()
        sup_queue = torch.zeros((unit_size, num_class - 1)).cuda()
    ...
    if torch.cuda.current_device() == 0:  # Save and print only on main GPU
        print(f"working time: {working_time:.2f}s, {total_frames / working_time:.2f}fps, {total_frames} frames")
