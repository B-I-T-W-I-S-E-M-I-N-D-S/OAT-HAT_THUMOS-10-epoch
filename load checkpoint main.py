...
def find_latest_checkpoint(opt):
    """
    Find the latest checkpoint file in the checkpoint directory
    Returns the epoch number to start from (0 if no checkpoints found)
    """
    checkpoint_dir = opt["checkpoint_path"]
    if not os.path.exists(checkpoint_dir):
        return 0, None
    
    # Look for checkpoint files with pattern: {exp}_checkpoint_{epoch}.pth.tar
    checkpoint_files = []
    for file in os.listdir(checkpoint_dir):
        if file.startswith(opt["exp"] + "_checkpoint_") and file.endswith(".pth.tar"):
            try:
                # Extract epoch number from filename
                epoch_str = file.replace(opt["exp"] + "_checkpoint_", "").replace(".pth.tar", "")
    ...
    return latest_epoch, checkpoint_path
...
def load_checkpoint(checkpoint_path, model, optimizer, scheduler):
    ...
    if 'best_map' in checkpoint:
        model.best_map = checkpoint['best_map']
        print(f"Best mAP loaded: {model.best_map}")
...
def train(opt): 
    ...
    start_epoch, checkpoint_path = find_latest_checkpoint(opt)
    
    if checkpoint_path is not None:
        # Load checkpoint and resume training
        loaded_epoch = load_checkpoint(checkpoint_path, model, optimizer, scheduler)
    ...
    for n_epoch in range(start_epoch, opt['epoch']):   
        if n_epoch >= 1:
            warmup = False
        
        print(f"Training epoch {n_epoch + 1}/{opt['epoch']}")
        ...
        state = {
            ...
            'best_map': getattr(model, 'best_map', 0.0)
        }
        
        checkpoint_file = opt["checkpoint_path"]+"/"+opt["exp"]+"_checkpoint_"+str(n_epoch+1)+".pth.tar"
        ...
        if IoUmAP_5 > model.best_map:
            ...
            best_checkpoint_file = opt["checkpoint_path"]+"/"+opt["exp"]+"_ckp_best.pth.tar"
            torch.save(best_state, best_checkpoint_file)
            print(f"New best model saved: {best_checkpoint_file} (mAP: {IoUmAP_5:.4f})")
